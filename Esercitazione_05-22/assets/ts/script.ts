const randomNum = (): number => {
  ...
  const result = (desk: number, player1: number, player2: number): void => {
    const difference1: number = Math.abs(desk - player1);
    const difference2: number = Math.abs(desk - player2);
...
console.log()
const deskNumber:number = randomNum();
